...
function renderSources(){const host=$('#sources');host.replaceChildren();for(const s of current.body.sources||[]){const row=make('div');row.className='source';const link=make('a',s.title||s.url);const url=safeURL(s.url);if(url){link.href=url;link.target='_blank';link.rel='noopener noreferrer';}row.append(link,make('br'),make('small',[s.source,s.ts].filter(Boolean).join(' · ')));host.append(row);}if(!host.children.length)host.append(make('p','Attach a news article or add a source link below.'));}
...
function attach(item){if(!current){openCase({id:crypto.randomUUID(),title:(item.title||'').slice(0,240),body:{sources:[],claims:[]}});}const existing=current.body.sources||[];const candidates=[item,...(Array.isArray(item.sources)?item.sources:[])];let added=0;for(const s of candidates){if(!s)continue;const url=safeURL(s.url);if(url&&!existing.some(x=>x.url===url)){existing.push({title:s.title||item.title,url,source:s.source||item.source,ts:s.ts||item.ts});added++;}}current.body.sources=existing;renderSources();caseTab('sources');if(added){changed();message('Source attached to the open case. Save to cloud to keep it.');}else message(existing.length?'This source is already attached.':'This entry has no valid source link. Add one manually.');}
function renderNews(){const q=$('#news-search').value.toLowerCase();const host=$('#news');host.replaceChildren();for(const item of news.filter(i=>(i.title||'').toLowerCase().includes(q)).slice(0,100)){const row=make('div');row.className='news-item';const text=make('div');const a=make('a',item.title);a.href='index.html#story='+encodeURIComponent(item.id);a.target='_blank';a.rel='noopener noreferrer';text.append(a,make('div',[item.source,item.ts].filter(Boolean).join(' · ')));const b=make('button','Attach to case');b.onclick=()=>{if(!busy)attach(item);};row.append(text,b);host.append(row);}if(!host.children.length)host.append(make('p','No matching articles. You can attach a source URL manually.'));}
async function loadNews(){try{const r=await fetch('feed.json',{cache:'no-store',signal:AbortSignal.timeout(20000)});if(!r.ok)throw Error();const data=await r.json();const rows=Array.isArray(data)?data:data.items;if(!Array.isArray(rows))throw Error();news=rows.filter(i=>i&&typeof i.title==='string'&&i.id!=null);$('#feed-status').textContent=data.generated?'Feed generated '+new Date(data.generated).toLocaleString():'Feed generation time unavailable.';const selected=new URLSearchParams(location.search).get('article'),item=news.find(i=>String(i.id)===selected);$('#incoming').hidden=!item;if(item){$('#incoming-title').textContent=item.title;$('#attach-incoming').onclick=()=>{if(!busy){attach(item);$('#editor').scrollIntoView({block:'start'});}};}else if(selected)message('The selected article is no longer in the feed. You can still attach its original URL manually.');renderNews();}catch{$('#feed-status').textContent='Live feed unavailable. Add source links manually or retry after the next feed update.';}}
for(const [key,title,hint]of fields){const l=make('label',title),a=make('textarea');a.name=key;a.placeholder=hint;l.append(a);$(key==='notes'?'#notes-field':'#analysis-fields').append(l);}
$('#login').onsubmit=e=>{e.preventDefault();run(async()=>{const password=$('#password').value;$('#password').value='';const data=await request('/auth/v1/token?grant_type=password','POST',{email:$('#email').value.trim(),password},false);session={...data,expires_at:data.expires_at||Date.now()/1000+data.expires_in};$('#login-panel').hidden=true;$('#workspace').hidden=false;$('#logout').hidden=false;message('Signed in. Loading your cases…');await loadCases();message('Workspace connected. Save explicitly; use Refresh to retrieve changes from another device.');await loadNews();await loadMarketDraft();});};
...
function caseTab(name){for(const key of ['sources','notes','ask']){$('#case-'+key).hidden=key!==name;document.querySelector(`[data-case-tab="${key}"]`).setAttribute('aria-pressed',String(key===name));}}
document.querySelectorAll('[data-case-tab]').forEach(b=>b.onclick=()=>caseTab(b.dataset.caseTab));
$('#question-to-notes').onclick=()=>{caseTab('notes');$('#editor').elements.notes.focus();};
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key==='s'&&current&&!busy){e.preventDefault();$('#editor').requestSubmit();}});
window.addEventListener('beforeunload',e=>{if(dirty){e.preventDefault();e.returnValue='';}});
async function loadMarketDraft(){
 const params=new URLSearchParams(location.search),country=params.get('market');
 if(!country||!Object.hasOwn(MarketCore.countries,country))return;
 const days=Number(params.get('days'));if(![1,7,14].includes(days))return;
 const r=await fetch('markets.json',{cache:'no-store',signal:AbortSignal.timeout(20000)});
 if(!r.ok)throw Error('Could not load the market snapshot. Open Markets and try again.');
 const data=await r.json();
 if(data.schema_version!==1||!Array.isArray(data.series))throw Error('Unsupported market dataset.');
 if(data.checked_at!==params.get('checked'))throw Error('The market dataset has updated since you opened it. Return to Markets, reload, and choose Write commentary again to use the figures shown there.');
 const snapshot=MarketCore.snapshot(data,country,days),sources=[];
 for(const s of snapshot.series){if(s.last){const url=s.last.source_url||s.source_url;if(safeURL(url)&&!sources.some(x=>x.url===url))sources.push({title:s.label,url,source:'Market data',ts:s.last.date});}}
 openCase({id:crypto.randomUUID(),title:`${MarketCore.countries[country]} — ${days===1?'daily':days===7?'weekly':'fortnightly'} market commentary · ${data.checked_at.slice(0,10)}`,body:{kind:'market_commentary',status:'Researching',market_snapshot:snapshot,sources,claims:[],event:`Data collection checked ${data.checked_at}. Figures below use each indicator’s latest available observation, not necessarily today.`,reaction:MarketCore.commentary(snapshot),notes:'Add your interpretation, supporting news, and alternative explanations. The saved market_snapshot preserves the observations used here.'}});
 changed();message('Market commentary draft created with a fixed data snapshot. Add your analysis and select Save to cloud. No causal explanation has been generated automatically.');
}
